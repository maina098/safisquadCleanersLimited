...
const formatMoney = (amount) => `KSh ${Number(amount || 0).toLocaleString('en-KE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
...
function App() {
  ...
  const [portalAudit, setPortalAudit] = useState([])
  const [portalSettlements, setPortalSettlements] = useState([])
  ...
  const [currentPage, setCurrentPage] = useState(() => ({ '/privacy-policy': 'privacy', '/terms': 'terms', '/faq': 'faq', '/portal': 'portal' }[window.location.pathname] || 'home'))
  ...
  useEffect(() => {
    const pageMeta = {
      ...
      portal: ['Portal | Safi Squad Operations', 'Secure admin portal for Safi Squad operations.', '/portal'],
    }[currentPage] || null
    if (!pageMeta) return
  ...
  const loadPortalDashboard = async () => {
    ...
    const role = localStorage.getItem('safisquad-portal-role') || 'SECRETARIAT'
    const canViewUsers = ['ADMIN', 'SUPER_ADMIN', 'MANAGEMENT'].includes(role)
    const canViewSettlements = canViewUsers || ['FINANCE', 'FINANCE_OFFICER'].includes(role)
    try {
      const [overviewResponse, auditResponse, settlementsResponse, usersResponse] = await Promise.all([
        fetch(`${apiBase}/api/admin/metrics`, { headers }),
        fetch(`${apiBase}/api/admin/audit`, { headers }),
        canViewSettlements ? fetch(`${apiBase}/api/settlements`, { headers }) : Promise.resolve(null),
        canViewUsers ? fetch(`${apiBase}/api/users`, { headers }) : Promise.resolve(null),
      ])
      ...
      const settlementsData = settlementsResponse?.ok ? await settlementsResponse.json() : null
      const usersData = usersResponse?.ok ? await usersResponse.json() : null

      if (overviewData) setPortalOverview(overviewData)
      if (auditData) setPortalAudit(auditData.audit || [])
      if (settlementsData) setPortalSettlements(settlementsData.settlements || [])
      if (usersData) setPortalUsers(usersData.users || [])
    } catch (error) {
      setPortalError(error.message || 'Some dashboard data could not be loaded.')
    }
  }
  const loadPortalMessages = async () => {
    const response = await fetch(`${apiBase}/api/messages`, { headers: portalHeaders() })
    if (response.ok) setPortalMessages((await response.json()).messages || [])
  }
  const loadMaintenance = async () => {
    const response = await fetch(`${apiBase}/api/system/maintenance`, { headers: portalHeaders() })
    if (response.ok) setMaintenance(await response.json())
  }
  const updateStatus = async (id, status) => { const order = orders.find((item) => Number(item.id) === Number(id)); if (!order) return false; const response = await fetch(`${apiBase}/api/orders/${id}/status`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', ...portalHeaders() }, body: JSON.stringify({ status, version: Number.isInteger(order.version) ? order.version : 0 }) }); if (response.ok) { loadOrders(); return true } if (response.status === 409) { setPortalError('This order was updated by another user. The latest order data has been loaded.'); await loadOrders(); return false } return false }
  const loginToPortal = async (event) => { event.preventDefault(); setPortalError(''); const formData = new FormData(event.currentTarget); const response = await fetch(`${apiBase}/api/auth/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Object.fromEntries(formData)) }); const data = await response.json().catch(() => ({})); if (!response.ok) { setPortalError(data.error || 'Login failed.'); return; } localStorage.setItem('safisquad-portal-session', 'active'); localStorage.setItem('safisquad-portal-token', data.token); localStorage.setItem('safisquad-portal-role', data.user.role); setPortalError(''); setPortalLoggedIn(true); setCurrentPage('portal'); loadOrders(); loadPortalDashboard(); loadPortalMessages(); loadMaintenance() }
  const logoutPortal = () => { localStorage.removeItem('safisquad-portal-session'); localStorage.removeItem('safisquad-portal-token'); localStorage.removeItem('safisquad-portal-role'); setPortalLoggedIn(false); setPortalError(''); setCurrentPage('home'); window.history.pushState(null, '', '/') }
  const statusOptions = ['Pending', 'Picked Up', 'In-Progress', 'QC Passed', 'Out for Delivery', 'Delivered']
  const overview = portalOverview?.overview || { totalOrders: 0, activeOrders: 0, pendingPickup: 0, qcQueue: 0, revenue: 0, statusBreakdown: {}, trend: [], recent: [] }
  const health = portalOverview?.health || { onTrack: 0, staffing: 0, backlog: 0 }
  const metricsCards = [
    { label: 'Active orders', value: overview.activeOrders, tone: 'cyan' },
    { label: 'Revenue', value: formatMoney(overview.revenue), tone: 'green' },
    { label: 'Pending pickup', value: overview.pendingPickup, tone: 'amber' },
    { label: 'QC queue', value: overview.qcQueue, tone: 'indigo' },
  ]
  const metricSeries = Array.isArray(overview.trend) && overview.trend.length ? overview.trend : [{ date: 'Today', amount: 0 }]
  const statusBreakdown = Object.entries(overview.statusBreakdown || {})
  const recentOrders = overview.recent && overview.recent.length ? overview.recent : orders.slice(0, 4)

  useEffect(() => {
    if (portalLoggedIn && currentPage === 'portal') {
      loadOrders()
      loadPortalDashboard()
      loadPortalMessages()
      loadMaintenance()
    }
  }, [portalLoggedIn, currentPage])

  useEffect(() => {
    if (!portalLoggedIn || currentPage !== 'portal') return undefined
    const token = localStorage.getItem('safisquad-portal-token')
    if (!token) return undefined
    const stream = new EventSource(`${apiBase}/api/dashboard/stream?token=${encodeURIComponent(token)}`)
    ...
    stream.onmessage = (message) => {
      ...
      if (event.event === 'query.raised') loadPortalMessages()
      if (event.event === 'system.maintenance' || event.event === 'system.restored') setMaintenance(event.payload)
      if (event.event.startsWith('order.')) { loadOrders(); loadPortalDashboard() }
    }
    return () => stream.close()
  }, [portalLoggedIn, currentPage])
  ...
  if (currentPage === 'portal') return <PortalPage loggedIn={portalLoggedIn} error={portalError} onLogin={loginToPortal} onLogout={logoutPortal} onBack={closePage} overview={overview} health={health} metricsCards={metricsCards} metricSeries={metricSeries} statusBreakdown={statusBreakdown} recentOrders={recentOrders} orders={orders} portalAudit={portalAudit} portalSettlements={portalSettlements} portalUsers={portalUsers} portalMessages={portalMessages} liveEvents={liveEvents} liveConnected={liveConnected} maintenance={maintenance} statusOptions={statusOptions} updateStatus={updateStatus} refresh={() => { loadOrders(); loadPortalDashboard(); loadPortalMessages(); loadMaintenance() }} formatMoney={formatMoney} />

  return <div className="site-shell">
    <header className="site-header"><a className="logo" href="#top" onClick={() => setCurrentPage('home')}><img src="/logo.jpeg" alt="Safi Squad Logo" className="logo-image" /><strong>safi<span>squad</span></strong></a><button className="menu-toggle" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle navigation" aria-expanded={menuOpen}>☰</button><nav className={menuOpen ? 'site-nav open' : 'site-nav'} role="navigation"><a href="#services">Services</a><a href="#pricing">Pricing</a><a href="https://drive.google.com/file/d/1b7dHZHil5TcrJPlo287xPMug0duB3E7o/view?usp=sharing" target="_blank" rel="noopener noreferrer">Detailed Prices</a><a href="#tracking">Track order</a><a href="#how-it-works">How it works</a><button onClick={() => {openPage('faq'); setMenuOpen(false)}} className="nav-link">FAQ</button><button onClick={() => {openPage('privacy'); setMenuOpen(false)}} className="nav-link">Privacy</button><button className="portal-link" onClick={() => { openPortal(); setMenuOpen(false) }} aria-label="Open admin portal">Portal</button><button className="nav-cta" onClick={() => openBooking()} aria-label="Book a pickup service">Book a pickup <span>↗</span></button></nav></header>
    <main id="top">
...
function PortalAuth({ error, onLogin, onBack }) {
  return <div className="portal-auth-page"><div className="portal-auth-brand"><span>S</span><strong>Safi Squad</strong><small>OPERATIONS PORTAL</small></div><div className="portal-auth-grid"><section className="portal-login-shell"><div className="auth-eyebrow">WELCOME BACK</div><h1>Operations,<br /><em>in focus.</em></h1><p className="portal-intro">Sign in to view bookings and manage the order journey from pickup to delivery.</p><form className="portal-form" onSubmit={onLogin}><label>Email address<input required name="email" type="email" autoComplete="username" placeholder="admin@safisquad.com" /></label><label>Password<input required name="password" type="password" autoComplete="current-password" placeholder="Enter your password" /></label><button className="button dark auth-submit" type="submit">Sign in to Portal <span>↗</span></button></form>{error && <p className="portal-error" role="alert">{error}</p>}<button className="portal-back-link" onClick={onBack}>Back to Safi Squad</button></section><aside className="auth-side-panel"><p className="kicker">ONE TEAM, FIVE VIEWS</p><h2>Everything your<br /><em>department needs.</em></h2><ul><li><strong>01</strong><span>See work that belongs to your role.</span></li><li><strong>02</strong><span>Coordinate through the shared inbox.</span></li><li><strong>03</strong><span>Stay current with live operational updates.</span></li></ul><p className="auth-security">Access is role-based. Accounts are provisioned by an authorized administrator.</p></aside></div></div>
}

function PortalPage({ loggedIn, error, onLogin, onLogout, onBack, overview, health, metricsCards, metricSeries, statusBreakdown, recentOrders, orders, portalAudit, portalSettlements, portalUsers, portalMessages, liveEvents, liveConnected, maintenance, statusOptions, updateStatus, refresh, formatMoney }) {
  if (!loggedIn) return <PortalAuth error={error} onLogin={onLogin} onBack={onBack} />

  return <><MaintenanceBanner state={maintenance} /><PortalWorkspace role={localStorage.getItem('safisquad-portal-role') || 'SECRETARIAT'} overview={overview} health={health} metricsCards={metricsCards} metricSeries={metricSeries} statusBreakdown={statusBreakdown} recentOrders={recentOrders} orders={orders} portalAudit={portalAudit} portalSettlements={portalSettlements} portalUsers={portalUsers} portalMessages={portalMessages} liveEvents={liveEvents} liveConnected={liveConnected} statusOptions={statusOptions} updateStatus={updateStatus} refresh={refresh} formatMoney={formatMoney} onLogout={onLogout} onBack={onBack} /></>

  return <div className="portal-page"><aside className="portal-sidebar"><strong className="portal-brand"><span>S</span> Safi Squad</strong><p>ADMIN PORTAL</p><button className="portal-nav-active">▦ Overview</button><button onClick={refresh}>▤ Orders</button><button>◷ Workflow</button><button>♙ Team</button><button>▥ Settlements</button><button className="portal-logout" onClick={onLogout}>Log out</button></aside><main className="portal-main"><header className="portal-topbar"><div><button className="portal-back-link" onClick={onBack}>← Public site</button><p className="kicker">ADMIN PORTAL / OVERVIEW</p><h1>Good morning,<br /><em>care team.</em></h1></div><div className="portal-user"><span>ADMIN</span><button onClick={onLogout}>Sign out</button></div></header><section className="portal-metric-grid">{metricsCards.map((card) => <article className={`portal-metric-card ${card.tone}`} key={card.label}><span>{card.label}</span><strong>{card.value}</strong><small>Live operational figure</small></article>)}</section><section className="portal-health-grid"><article><span>Operational health</span><strong>{health.onTrack}%</strong><small>Orders on track</small></article><article><span>Staffing coverage</span><strong>{health.staffing}%</strong><small>Available capacity</small></article><article><span>Scheduling backlog</span><strong>{health.backlog}</strong><small>Orders to schedule</small></article><article><span>Total orders</span><strong>{overview.totalOrders}</strong><small>All recorded bookings</small></article></section><div className="portal-content-grid"><section className="portal-panel"><div className="portal-section-heading"><div><p className="kicker">THROUGHPUT</p><h2>Revenue trend</h2></div><button onClick={refresh}>Refresh ↻</button></div><div className="portal-trend">{metricSeries.map((point) => { const maxAmount = Math.max(...metricSeries.map((item) => Number(item.amount || 0)), 1); return <div className="portal-trend-bar" key={point.date}><i style={{ height: `${Math.max(10, Number(point.amount || 0) / maxAmount * 100)}%` }} /><span>{point.date.slice(5).replace('-', '/')}</span><small>{formatMoney(point.amount || 0)}</small></div> })}</div></section><section className="portal-panel"><div className="portal-section-heading"><div><p className="kicker">WORKFLOW</p><h2>Status mix</h2></div><span>{overview.totalOrders} total</span></div><div className="portal-breakdown">{statusBreakdown.map(([status, count]) => <div className="portal-breakdown-row" key={status}><span>{status}</span><div><i style={{ width: `${Math.max(7, count / Math.max(1, overview.totalOrders) * 100)}%` }} /></div><strong>{count}</strong></div>)}</div></section></div><div className="portal-content-grid lower"><section className="portal-panel"><div className="portal-section-heading"><div><p className="kicker">SERVICE PIPELINE</p><h2>Recent orders</h2></div><button onClick={refresh}>Refresh ↻</button></div><div className="portal-orders">{recentOrders.length ? recentOrders.map((order) => <article className="admin-order" key={order.id || order.tracking_code}><div><strong>{order.tracking_code}</strong><span>{order.customer_name} · {order.service}</span><small>{order.preferred_date || 'No date'} · {formatMoney(order.estimated_cost || 0)}</small></div><div className="admin-order-meta"><span className="admin-status">{order.status}</span><select value={order.status} onChange={(event) => updateStatus(order.id, event.target.value)}>{statusOptions.map((status) => <option key={status}>{status}</option>)}</select></div></article>) : <p className="portal-empty">No recent orders available.</p>}</div></section><section className="portal-panel"><div className="portal-section-heading"><div><p className="kicker">CONTROL LOG</p><h2>Audit trail</h2></div><span>{portalAudit.length} events</span></div><div className="portal-audit">{portalAudit.slice(0, 8).map((entry) => <div className="audit-row" key={entry.id || `${entry.action}-${entry.created_at}`}><strong>{entry.action}</strong><span>{new Date(entry.created_at || Date.now()).toLocaleString()}</span></div>)}{!portalAudit.length && <p className="portal-empty">No audit events yet.</p>}</div></section></div></main></div>
}

function MaintenanceBanner({ state = {} }) {
  if (!state.enabled) return null
  return <div className="banner-warn" role="status">{state.message || 'Maintenance mode is active.'}</div>
}

function LiveCommunication({ portalMessages = [], liveEvents = [], liveConnected = false }) {
  return <section className="portal-communication" aria-label="Live department communication"><div className="portal-content-grid"><section className="portal-panel"><div className="portal-section-heading"><div><p className="kicker">SHARED EVENT BUS</p><h2>Live activity</h2></div><span>{liveConnected ? 'Connected' : 'Reconnecting'}</span></div>{liveEvents.slice(0, 4).map((event) => <div className="audit-row" key={event.id}><strong>{event.event}</strong><span>{new Date(event.at).toLocaleTimeString()}</span></div>)}</section><section className="portal-panel"><div className="portal-section-heading"><div><p className="kicker">INTER-DEPARTMENT INBOX</p><h2>Queries</h2></div><span>{portalMessages.filter((message) => message.status === 'UNREAD').length} unread</span></div>{portalMessages.slice(0, 4).map((message) => <article className="audit-row" key={message.id}><strong>{message.subject}</strong><span>{message.from_dept} → {message.to_dept} · {message.priority}</span></article>)}</section></div></section>
}

function PortalWorkspace({ role, overview, health, metricsCards, metricSeries, statusBreakdown, orders, portalAudit, portalSettlements, portalUsers, portalMessages, liveEvents, liveConnected, statusOptions, updateStatus, refresh, formatMoney, onLogout, onBack }) {
  const [activeSection, setActiveSection] = useState('overview')
  const [campaigns, setCampaigns] = useState([{ name: 'September garment care offer', channel: 'WhatsApp', status: 'Draft', budget: 0 }])
  const actionMap = { Receive: 'Picked Up', Acknowledge: 'In-Progress', Clean: 'QC Passed', Finish: 'Out for Delivery', Deliver: 'Delivered' }
  const actionNames = Object.keys(actionMap)
  const allSections = [{ id: 'overview', label: 'Overview', icon: '▦', roles: ['ADMIN', 'SUPER_ADMIN', 'MANAGEMENT', 'SECRETARIAT', 'MEMBER', 'FINANCE', 'FINANCE_OFFICER', 'PROMOTIONS', 'TECHNICAL'] }, { id: 'orders', label: 'Orders', icon: '▤', roles: ['ADMIN', 'SUPER_ADMIN', 'MANAGEMENT', 'SECRETARIAT', 'MEMBER'] }, { id: 'finance', label: 'Finance', icon: '▥', roles: ['ADMIN', 'SUPER_ADMIN', 'MANAGEMENT', 'FINANCE', 'FINANCE_OFFICER'] }, { id: 'team', label: 'Team', icon: '♙', roles: ['ADMIN', 'SUPER_ADMIN', 'MANAGEMENT'] }, { id: 'marketing', label: 'Marketing', icon: '✦', roles: ['ADMIN', 'SUPER_ADMIN', 'MANAGEMENT', 'PROMOTIONS'] }, { id: 'technical', label: 'Technical', icon: '◷', roles: ['ADMIN', 'SUPER_ADMIN', 'MANAGEMENT', 'TECHNICAL'] }]
  const sections = [{ id: 'notifications', label: 'Notifications', icon: '◉' }, ...allSections.filter((section) => section.roles.includes(role))]
  const addCampaign = () => setCampaigns([...campaigns, { name: 'New promotion', channel: 'Instagram', status: 'Draft', budget: 0 }])
  const updateCampaign = (index, field, value) => setCampaigns(campaigns.map((campaign, campaignIndex) => campaignIndex === index ? { ...campaign, [field]: value } : campaign))
  const renderOrders = () => <section className="portal-panel portal-wide-panel"><div className="portal-section-heading"><div><p className="kicker">SECRETARY WORK QUEUE</p><h2>All orders and tracking</h2></div><button onClick={refresh}>Refresh ↻</button></div><p className="portal-section-copy">Secretaries can track each booking and mark the operational handoff as it happens.</p><div className="portal-table-wrap"><table className="portal-table"><thead><tr><th>Tracking</th><th>Customer</th><th>Service</th><th>Status</th><th>Secretary actions</th></tr></thead><tbody>{orders.length ? orders.map((order) => <tr key={order.id || order.tracking_code}><td><strong>{order.tracking_code}</strong><small>{order.preferred_date || 'No date'}</small></td><td>{order.customer_name}<small>{order.address || 'Address unavailable'}</small></td><td>{order.service}<small>{formatMoney(order.estimated_cost || 0)}</small></td><td><span className="admin-status">{order.status}</span><select value={order.status} onChange={(event) => updateStatus(order.id, event.target.value)}>{statusOptions.map((status) => <option key={status}>{status}</option>)}</select></td><td><div className="order-actions">{actionNames.map((action) => <button key={action} disabled={order.status === actionMap[action]} onClick={() => updateStatus(order.id, actionMap[action])}>{action}</button>)}</div></td></tr>) : <tr><td colSpan="5" className="portal-empty">No orders available.</td></tr>}</tbody></table></div></section>
  const renderFinance = () => <section className="portal-panel portal-wide-panel"><div className="portal-section-heading"><div><p className="kicker">FINANCE OFFICER CONTROL</p><h2>Money, usage and appropriations</h2></div><button onClick={refresh}>Refresh ↻</button></div><div className="finance-summary"><article><span>Total received</span><strong>{formatMoney(overview.revenue || 0)}</strong></article><article><span>Settlement runs</span><strong>{portalSettlements.length}</strong></article><article><span>Appropriated for payout</span><strong>{formatMoney(portalSettlements.reduce((total, item) => Math.round((total + Number(item.distributable_amount || 0)) * 100) / 100, 0))}</strong></article></div><div className="portal-table-wrap"><table className="portal-table"><thead><tr><th>Period</th><th>Money received</th><th>Restock usage</th><th>Contingency</th><th>Growth appropriation</th><th>Distributable</th><th>Status</th></tr></thead><tbody>{portalSettlements.length ? portalSettlements.map((settlement) => <tr key={settlement.id || settlement.week_start}><td>{settlement.week_start}</td><td>{formatMoney(settlement.revenue || 0)}</td><td>{formatMoney(settlement.restock_allocation || 0)}</td><td>{formatMoney(settlement.contingency_buffer || 0)}</td><td>{formatMoney(settlement.growth_fund || 0)}</td><td><strong>{formatMoney(settlement.distributable_amount || 0)}</strong></td><td><span className="admin-status">{settlement.status || 'PENDING_REVIEW'}</span></td></tr>) : <tr><td colSpan="7" className="portal-empty">No settlement records available.</td></tr>}</tbody></table></div></section>
  const renderTeam = () => <section className="portal-panel portal-wide-panel"><div className="portal-section-heading"><div><p className="kicker">PEOPLE DIRECTORY</p><h2>Team members</h2></div><span>{portalUsers.length} members</span></div><div className="team-grid">{portalUsers.length ? portalUsers.map((member) => <article className="team-member" key={member.id || member.email}><span className="team-avatar">{member.email?.slice(0, 1).toUpperCase()}</span><div><strong>{member.email}</strong><small>{member.phone || 'Phone not recorded'}</small></div><b>{member.role}</b></article>) : <p className="portal-empty">No team records available.</p>}</div></section>
  const renderMarketing = () => <section className="portal-panel portal-wide-panel"><div className="portal-section-heading"><div><p className="kicker">MARKETING TEAM</p><h2>Promotions and advertisements</h2></div><button onClick={addCampaign}>+ New promotion</button></div><p className="portal-section-copy">Prepare campaigns, assign a channel, record spend and keep launch approval visible.</p><div className="campaign-list">{campaigns.map((campaign, index) => <article className="campaign-row" key={`${campaign.name}-${index}`}><input value={campaign.name} onChange={(event) => updateCampaign(index, 'name', event.target.value)} aria-label="Promotion name" /><select value={campaign.channel} onChange={(event) => updateCampaign(index, 'channel', event.target.value)}><option>WhatsApp</option><option>Instagram</option><option>Facebook</option><option>Google</option><option>Print</option></select><input type="number" min="0" value={campaign.budget} onChange={(event) => updateCampaign(index, 'budget', event.target.value)} aria-label="Campaign budget" /><select value={campaign.status} onChange={(event) => updateCampaign(index, 'status', event.target.value)}><option>Draft</option><option>Ready for approval</option><option>Live</option><option>Paused</option></select></article>)}</div></section>
  const renderTechnical = () => <section className="portal-panel portal-wide-panel"><div className="portal-section-heading"><div><p className="kicker">TECHNICAL STAFF</p><h2>System logs and emergency response</h2></div><button onClick={refresh}>Refresh logs ↻</button></div><div className="technical-grid"><div><h3>Recent system logs</h3><div className="portal-audit">{portalAudit.length ? portalAudit.slice(0, 12).map((entry) => <div className="audit-row" key={entry.id || `${entry.action}-${entry.created_at}`}><strong>{entry.action}</strong><span>{entry.entity_type} · {new Date(entry.created_at || Date.now()).toLocaleString()}</span></div>) : <p className="portal-empty">No system logs available.</p>}</div></div><div className="runbook"><h3>Emergency runbook</h3><ol><li>Confirm the incident, affected service and time.</li><li>Protect customer data: stop unsafe access and preserve logs.</li><li>Check API health, database connectivity and background jobs.</li><li>Communicate impact to the administrator and operations lead.</li><li>Record the resolution, owner and follow-up action in the audit trail.</li></ol><p>Do not delete logs, expose credentials, or restart production services without authorization.</p></div></div></section>
  const renderNotifications = () => <LiveCommunication portalMessages={portalMessages} liveEvents={liveEvents} liveConnected={liveConnected} />
  if (activeSection === 'notifications') return <div className="portal-page portal-notifications-view"><aside className="portal-sidebar"><strong className="portal-brand"><span>S</span> Safi Squad</strong><p>ADMIN PORTAL</p>{sections.map((section) => <button className={activeSection === section.id ? 'portal-nav-active' : ''} onClick={() => setActiveSection(section.id)} key={section.id}>{section.icon} {section.label}</button>)}<button className="portal-logout" onClick={onLogout}>Log out</button></aside><main className="portal-main"><header className="portal-topbar"><div><button className="portal-back-link" onClick={() => setActiveSection('overview')}>← Dashboard overview</button><p className="kicker">OPERATIONS PORTAL / NOTIFICATIONS</p><h1>Live activity and<br /><em>department queries.</em></h1></div><div className="portal-user"><span>NOTIFICATIONS</span><button onClick={onLogout}>Sign out</button></div></header>{renderNotifications()}</main></div>
  return <div className="portal-page"><aside className="portal-sidebar"><strong className="portal-brand"><span>S</span> Safi Squad</strong><p>ADMIN PORTAL</p>{sections.map((section) => <button className={activeSection === section.id ? 'portal-nav-active' : ''} onClick={() => setActiveSection(section.id)} key={section.id}>{section.icon} {section.label}</button>)}<button className="portal-logout" onClick={onLogout}>Log out</button></aside><main className="portal-main"><header className="portal-topbar"><div><button className="portal-back-link" onClick={onBack}>← Public site</button><p className="kicker">ADMIN PORTAL / {activeSection.toUpperCase()}</p><h1>{sections.find((section) => section.id === activeSection)?.label || 'Overview'}<br /><em>control room.</em></h1></div><div className="portal-user"><span>ADMIN</span><button onClick={onLogout}>Sign out</button></div></header>{activeSection === 'overview' && <><section className="portal-metric-grid">{metricsCards.map((card) => <article className={`portal-metric-card ${card.tone}`} key={card.label}><span>{card.label}</span><strong>{card.value}</strong><small>Live operational figure</small></article>)}</section><section className="portal-health-grid"><article><span>Operational health</span><strong>{health.onTrack}%</strong><small>Orders on track</small></article><article><span>Staffing coverage</span><strong>{health.staffing}%</strong><small>Available capacity</small></article><article><span>Scheduling backlog</span><strong>{health.backlog}</strong><small>Orders to schedule</small></article><article><span>Total orders</span><strong>{overview.totalOrders}</strong><small>All recorded bookings</small></article></section><div className="portal-content-grid"><section className="portal-panel"><div className="portal-section-heading"><div><p className="kicker">THROUGHPUT</p><h2>Revenue trend</h2></div><button onClick={refresh}>Refresh ↻</button></div><div className="portal-trend">{metricSeries.map((point) => { const maxAmount = Math.max(...metricSeries.map((item) => Number(item.amount || 0)), 1); return <div className="portal-trend-bar" key={point.date}><i style={{ height: `${Math.max(10, Number(point.amount || 0) / maxAmount * 100)}%` }} /><span>{point.date.slice(5).replace('-', '/')}</span></div> })}</div></section><section className="portal-panel"><div className="portal-section-heading"><div><p className="kicker">WORKFLOW</p><h2>Status mix</h2></div><span>{overview.totalOrders} total</span></div><div className="portal-breakdown">{statusBreakdown.map(([status, count]) => <div className="portal-breakdown-row" key={status}><span>{status}</span><div><i style={{ width: `${Math.max(7, count / Math.max(1, overview.totalOrders) * 100)}%` }} /></div><strong>{count}</strong></div>)}</div></section></div></>}{activeSection === 'orders' && renderOrders()}{activeSection === 'finance' && renderFinance()}{activeSection === 'team' && renderTeam()}{activeSection === 'marketing' && renderMarketing()}{activeSection === 'technical' && renderTechnical()}</main></div>
}
